...
2. When an in-house examination method, previously validated by the laboratory, has since undergone modification that is minor in extent (see section on :ref:`Ongoing Assessment <ongoing-assessment>` for guidance). Here the role of verification is to check that the update to the examination method has not negatively affected the performance criteria of the examination method, and that it remains fit for clinical purpose. 
